    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                const float t = (cos(_Time.y * UNITY_PI * _RepeatTime * 2) * 0.5 + 0.5) * 0.1 + _Time.y * _RepeatTime;
                float y = abs(uv.y - 0.5) + t;
                y += cos(i.objPos.z * 0.2);
                uv.y = y;

                float f = Luminance(tex2D(_MainTex, uv).xyz);

                const float distToEdge = min(i.uv.x, 1 - i.uv.x);
                ...
                float3 col = lerp(rainbow(i.objPos.z * 0.02), 1, 0.2);
                col = lerp(col, palette(i.objPos.x * 0.3, 0.5, 0.5, 1, float3(0, 0.1, 0.2)), 0.4);
                col *= f;
